...
/// \author Antonio Palasciano <antonio.palasciano@cern.ch>, Università degli
/// Studi di Bari & INFN, Sezione di Bari
...
void customize(std::vector<o2::framework::ConfigParamSpec> &workflowOptions) {
  ConfigParamSpec optionDoMC{
      "doMC", VariantType::Bool, false, {"Perform MC matching."}};
...
struct HFCandidateCreatorBPlus {
  ...
  Configurable<double> d_minrelchi2change{
      "d_minrelchi2change", 0.9, "stop iterations if chi2/chi2old > this"};
  Configurable<bool> d_UseAbsDCA{"d_UseAbsDCA", true, "Use Abs DCAs"};

  OutputObj<TH1F> hCovPVXX{TH1F("hCovPVXX",
                                "2-prong candidates;XX element of cov. matrix "
  ...
  Configurable<int> d_selectionFlagD0{"d_selectionFlagD0", 1,
                                      "Selection Flag for D0"};
  Configurable<int> d_selectionFlagD0bar{"d_selectionFlagD0bar", 1,
                                         "Selection Flag for D0bar"};
  Configurable<double> cutEtaCandMax{"cutEtaCandMax", 1.,
                                     "max. cand. pseudorapidity"};
  Filter filterSelectCandidates =
      (aod::hf_selcandidate_d0::isSelD0 >= d_selectionFlagD0 ||
       aod::hf_selcandidate_d0::isSelD0bar >= d_selectionFlagD0bar);

  void process(
      aod::Collision const &collisions,
      soa::Filtered<soa::Join<aod::HfCandProng2, aod::HFSelD0Candidate>> const
          &candidates,
    ...
    for (auto &candidate : candidates) {
      ...
      if ((candidate.isSelD0bar() < d_selectionFlagD0bar) &&
          (candidate.isSelD0() < d_selectionFlagD0))
        continue;
      ...
      const std::array<float, 3> vertexD0 = {candidate.xSecondaryVertex(),
                                             candidate.ySecondaryVertex(),
                                             candidate.zSecondaryVertex()};
      const std::array<float, 3> momentumD0 = {candidate.px(), candidate.py(),
                                               candidate.pz()};
      ...
      // LOGF(INFO, "All track: %d (prong0); %d (prong1)",
      // candidate.index0().globalIndex(), candidate.index1().globalIndex());
      // LOGF(INFO, "All track pT: %f (prong0); %f (prong1)", prong0.pt(),
      // prong1.pt());
      ...
      auto trackD0 = o2::dataformats::V0(vertexD0, momentumD0, D0CovMatrix,
                                         prong0TrackParCov, prong1TrackParCov,
                                         {0, 0}, {0, 0});
      ...
      for (auto &track : tracks) {
        // if (count % 100 == 0) {
        //  LOGF(INFO, "Col: %d (cand); %d (track)", candidate.collisionId(),
        //  track.collisionId()); count++;
        // }
        ...
        if (candidate.index0Id() == track.globalIndex() ||
            candidate.index1Id() == track.globalIndex())
        ...
        bfitter.propagateTracksToVertex(); // propagate the bachelor and D0 to
                                           // the B+ vertex
        bfitter.getTrack(0).getPxPyPzGlo(
            pvecD0); // momentum of D0 at the B+ vertex
        bfitter.getTrack(1).getPxPyPzGlo(
            pvecbach); // momentum of pi+ at the B+ vertex
        ...
        hCovSVXX->Fill(
            covMatrixPCA[0]); // FIXME: Calculation of errorDecayLength(XY)
                              // gives wrong values without this line.

        pvecBCand = array{pvecbach[0] + pvecD0[0], pvecbach[1] + pvecD0[1],
                          pvecbach[2] + pvecD0[2]};
        ...
        bfitter.getTrack(0).propagateToDCA(primaryVertex, d_bz,
                                           &impactParameter0);
        bfitter.getTrack(1).propagateToDCA(primaryVertex, d_bz,
                                           &impactParameter1);
        ...
        getPointDirection(
            ...
            BSecVertex, phi, theta);
        auto errorDecayLength =
            std::sqrt(getRotatedCovMatrixXX(covMatrixPV, phi, theta) +
                      getRotatedCovMatrixXX(covMatrixPCA, phi, theta));
        auto errorDecayLengthXY =
        ...
        rowCandidateBase(
            collision.globalIndex(), collision.posX(), collision.posY(),
            collision.posZ(), BSecVertex[0], BSecVertex[1], BSecVertex[2],
            errorDecayLength, errorDecayLengthXY, chi2PCA, pvecD0[0], pvecD0[1],
            pvecD0[2], pvecbach[0], pvecbach[1], pvecbach[2],
            impactParameter0.getY(), impactParameter1.getY(),
            std::sqrt(impactParameter0.getSigmaY2()),
            std::sqrt(impactParameter1.getSigmaY2()), candidate.globalIndex(),
            track.globalIndex(), // index D0 and bachelor
            hfFlag);
...
struct HFCandidateCreatorBPlusMC {
  ...
  void process(aod::HfCandBPlus const &candidates, aod::HfCandProng2,
    ...
    for (auto &candidate : candidates) {
      ...
      auto arrayDaughters = array{candidate.index1_as<aod::BigTracksMC>(),
                                  D0barTrack.index0_as<aod::BigTracksMC>(),
                                  D0barTrack.index1_as<aod::BigTracksMC>()};

      // B± → D0bar(D0) π± → (K± π∓) π±
      // Printf("Checking B± → D0(bar) π±");
      indexRec = RecoDecay::getMatchedMCRec(particlesMC, arrayDaughters, 521,
                                            array{321, -kPiPlus, +kPiPlus},
                                            true, &sign, 2);
      if (indexRec > -1) {
    ...
    for (auto &particle : particlesMC) {
      ...
      // B± → D0bar(D0) π± → (K± π∓) π±
      // Printf("Checking B± → D0(bar) π±");
      if (RecoDecay::isMatchedMCGen(particlesMC, particle, 521,
                                    array{-421, +kPiPlus}, true, &sign, 1)) {
        // D0(bar) → π± K∓
        // Printf("Checking D0(bar) → π± K∓");
        std::vector<int> arrayDaughterB;
        RecoDecay::getDaughters(particlesMC, particle, &arrayDaughterB,
                                array{421}, 1); // it takes abs. of PDG codes
        auto D0candidateMC = particlesMC.iteratorAt(arrayDaughterB[0]);
        if (RecoDecay::isMatchedMCGen(particlesMC, D0candidateMC, 421,
                                      array{-321, +kPiPlus}, true, &sign)) {
...
WorkflowSpec defineDataProcessing(ConfigContext const &cfgc) {
  WorkflowSpec workflow{
      adaptAnalysisTask<HFCandidateCreatorBPlus>(
          cfgc, TaskName{"hf-cand-creator-bplus"}),
      adaptAnalysisTask<HFCandidateCreatorBPlusExpressions>(
          cfgc, TaskName{"hf-cand-creator-bplus-expressions"})};
  const bool doMC = cfgc.options().get<bool>("doMC");
  if (doMC) {
    workflow.push_back(adaptAnalysisTask<HFCandidateCreatorBPlusMC>(
        cfgc, TaskName{"hf-cand-creator-bplus-mc"}));
  }
  return workflow;
}